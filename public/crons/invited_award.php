...
$sql = 'SELECT * FROM t_user_invited WHERE create_time >= ? AND create_time < ?';
$invited1 = $db->getAll($sql, date('Y-m-d 00:00:00', strtotime('-1 day')), date('Y-m-d 00:00:00'));
foreach ($invited1 as $info1) {
    $sql = 'SELECT * FROM t_walk WHERE user_id = ? AND user_id = ';
}
